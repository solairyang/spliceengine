    ...
    public DataSet<ExecRow> write() throws StandardException{
        ...
        try{
            ...
            if(operation instanceof InsertOperation){
                ...
                for(SpliceOperation op:ops){
                    ...
                    if (brr != null) {
                        ...
                        badFileNames = op.getOperationContext().getBadRecordsRecorder().getBadRecordFileName().toString();
                    }
                    if(badFileNames!="") badFileNames += " ; ";
                    badFileNames += op.getOperationContext().getBadRecordsRecorder().getBadRecordFileName().toString();
                }
                ...
                if(badRecords > 0){
                    operationContext.getActivation().getLanguageConnectionContext().setBadFile(badFileNames);
                    if (insertOperation.isAboveFailThreshold(badRecords)) {
                        throw ErrorState.LANG_IMPORT_TOO_MANY_BAD_RECORDS.newException(badFileNames);
                    }
                }
            }
            ...
            if (badRecords > 0) {
                ...
                valueRow.setColumn(3, new SQLVarchar(badFileNames));
            }
            else {
